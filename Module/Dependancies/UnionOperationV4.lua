    ...
    if typeof(assetId)=="string" then
        ...
        if id then
            ...
            if findCache then
                ...
                loadable=self:applyChildData(childData,isIntersection);
...
function mod:parseVarientData(data:string,isIntersection)
    ...
    elseif magic=="<roblo" then -- this ones a bit odd, MeshData2 can contain a RBXM blob, so we have to check for it
        return self:applyChildData(data,isIntersection);
    end;
end;
function mod:applyAssetData(assetData:string,isIntersection)
    ...
    return self:applyChildData(childData,isIntersection);
end;
--
...
local function centerUnionPivotBase(union,parent)
