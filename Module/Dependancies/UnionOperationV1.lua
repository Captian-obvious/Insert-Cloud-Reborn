...
function mod:applyAssetId(assetId:string,isIntersection)
    local loadable=nil;
    ...
    if typeof(assetId)=="string" then
        ...
        if id then
            ...
            if findCache then
                ...
                loadable=mod:applyChildData(childData,isIntersection);
...
function mod:parseMeshData(meshdata:string,isIntersection)
    local buf=BufferedReader(self.modules.b64.decode(meshdata));
    ...
    elseif magic=="<roblo" then -- this ones a bit odd, MeshData2 can contain a RBXM blob, so we have to check for it
        return self:applyChildData(meshdata,isIntersection);
    end;
end;
--
        ...
        local function reconstruct(model)
            ...
            for i,v in pairs(model:GetChildren()) do
                if v:IsA("BasePart") then
                    if ((v:IsA("BasePart") and not v:GetAttribute("IsNegateOperation")) and not partToAttachTo) then
                        partToAttachTo=v;
                    end;
